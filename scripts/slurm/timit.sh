...
check_and_do "$gpu_opts --ntasks=1 --gpus-per-task=1" -s 2

( 
  sleep 1
  check_and_do "$gpu_opts --ntasks=1 --gpus-per-task=1" -s 3 -z partial
  check_and_do "$gpu_opts --ntasks=1 --gpus-per-task=1" -s 4 -z 'indep partial' -p pretrain
  check_and_do "$cpu_opts --ntasks=1 --cpus-per-task=8" -s 5 -z 'indep partial' -p pretrain
) & outer_pids[$!]="indep_partial_pretrain"
(
  sleep 1
  check_and_do "$gpu_opts --ntasks=1 --gpus-per-task=1" -s 4 -z 'indep partial' -p flatstart
  check_and_do "$cpu_opts --ntasks=1 --cpus-per-task=8" -s 5 -z 'indep partial' -p flatstart -c cpu
) & outer_pids[$!]="indep_partial_flatstart"
(
  sleep 1
  check_and_do "$gpu_opts --ntasks=1 --gpus-per-task=2" -s 3 -z full
  check_and_do "$gpu_opts --ntasks=1 --gpus-per-task=2" -s 4 -e 'srswor ais-c' -p pretrain
  check_and_do "$gpu_tasks --ntasks=1 --gpus-per-task=4" -s 4 -e 'sf-biased sf-is direct' -p pretrain
  check_and_do "$cpu_opts --ntasks=1 --cpus-per-task=8" -s 5 -e 'sf-biased sf-is direct' -p pretrain -c cpu
) & outer_pids[$!]="full_pretrain"
(
  sleep 1
  check_and_do "$gpu_opts --ntasks=1 --gpus-per-task=2" -s 4 -e 'srswor ais-c' -p flatstart
  check_and_do "$gpu_tasks --ntasks=1 --gpus-per-task=4" -s 4 -e 'sf-biased sf-is direct' -p flatstart
