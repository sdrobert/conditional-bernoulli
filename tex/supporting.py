...
timit_ppu = 38
timit_sent_dur = 3.
phone_secs = timit_sent_dur / timit_ppu
frame_secs = 0.03
...
def add_probs_to_fig(
    ...
    if include_expected:
        plt.hlines(
            ...
            linestyle="--",
            color=s.get_ec() if expected_match else "black",
        )
    if include_L:
        plt.vlines(
            t[L - 1],
...
def compare_probs(label_secs, frame_secs, p, *rest, total_secs=3.0):
    import matplotlib.pyplot as plt
...
def compare_shifts(label_secs, frame_secs, *rest, total_secs=3.0):
    import matplotlib.pyplot as plt
