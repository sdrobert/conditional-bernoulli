...
#     w = w.transpose(0, -1).flatten(end_dim=-2)  # (N, T)
#     b = [torch.full_like(w[..., 0], -1, dtype=int)]  # [(N,)]
#     Rhist = Rhist.flatten(2).transpose(0, 1).transpose(1, 2).flip(2)
#     t = torch.arange(T, dtype=int, device=w.device)
#     # Rhist[c, :, t] = R(c, A^c_{t + 1})
...
#         t_lm1 = b[-1]
#         p = (
#             (w * R_Lml[:, 1:]).masked_fill((t_lm1.unsqueeze(-1) >= t), 0.)
#             .masked_fill((Lml < 0).unsqueeze(-1), 1.)
#         )

#         R_Lmlp1 = Rhist.gather(
#             0, (Lml + 1).clamp(0).unsqueeze(0).unsqueeze(-1).expand_as(Rhist[1:]))[0]
#         R_Lmlp1_tlm1 = R_Lmlp1.gather(1, (t_lm1 + 1).unsqueeze(-1))[..., 0]

#         assert torch.all(torch.isclose(p.sum(1), R_Lmlp1_tlm1) | (Lml < 0))
