...
def test_count_history(count, device, batch_size):
    ...
    C_exp = torch.stack(
        [
            ...
            torch.stack([zeros, w1, w1 + w2, w1 + w2 + w3, w1 + w2 + w3 + w4], 1),
            torch.stack(
                [
                    zeros,
                    zeros,
...
def test_count_properties(count, device):
    ...
    C_sub_j = count(w_sub_j, L).transpose(0, 1)
    C_L_act = (C_sub_j[:, -2] * w).sum(0) / L
    C_exp = count(w.unsqueeze(0), L).transpose(0, 1)
    C_L_exp = C_exp[:, -1]
    ...
    w_left = w[: T // 2]
    w_right = w[T // 2 :]
    C_left = count(w_left.unsqueeze(0), L).transpose(0, 1)
    C_right = count(w_right.unsqueeze(0), L).transpose(0, 1)
...
def test_log_count_values(log_count, device):
    T, L, N = 10, 3, 30
    w = torch.randn((N, T), device=device).exp()
    w.requires_grad_(True)
    C_exp = count_function.count(w, L)
    (g_exp,) = torch.autograd.grad(C_exp, w, torch.ones_like(C_exp))
    C_act = log_count(w.log(), L).exp()
